  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      const fetchedMovies = await axios.get(
        `https://api.themoviedb.org/3/movie/${id}?api_key=e83c60df8d74070453c0e741dfb15119&language=en-US`
      );
      // setItems();
      ...
      setItems(sample);
      setGenresSeries(movieItem.genres);
    ...
    const fetchVideoSeries = async () => {
      try {
        const fetchedVideoSeries = await tmdb.get(
          `https://api.themoviedb.org/3/movie/${id}/videos?api_key=e83c60df8d74070453c0e741dfb15119&language=en-US`
        );
